...
abstract: >
  Obstruent consonants often exert a localized influence on pitch level in following segments, such that pitch is higher following phonologically voiceless obstruents. The precise underlying mechanisms of such co-intrinsic pitch effects remain disputed, and it remains unclear how they interact with other (more or less localized) demands on the larynx. We revisit co-intrinsic pitch effects in Danish, a language with two series of actively devoiced stops that differ in the magnitude and extent of glottal spreading, and accordingly, in the magnitude and extent of positive voice onset time. Danish is further interesting because it has a phonological voice quality contrast (stød) which places complex demands on the larynx, and there is regional variation in where intonational pitch peaks occur relative to the stressed syllable. We elicit alternative question sentences from speakers of two different dialects of Danish, which allows us to tease apart how co-intrinsic pitch effects behave when there are multiple series of voiceless stops, when the global pitch target is high versus low, when a high pitch target falls early versus late in a focused word, and when there are competing demands on the vocal fold from a voice quality contrast. We find that pitch is uniformly raised in both series of stops relative to a neutral baseline, but the duration and magnitude of the effect differs by stop category. This effect is enhanced by a global intonational high pitch target in words with modal voice but inhibited by competing demands on the larynx in words with stød.
---
...
Co-intrinsic *F*~0~ of consonants [which we refer to as C*F*~0~ following e.g. @dicristo1986], i.e. the effect that consonants have on the *F*~0~ of surrounding segments, has been documented in many languages with various kinds of laryngeal contrasts in obstruents [e.g. @chen2011jphon; @kirby2018jphon].
C*F*~0~ is assumed to play a major role in tonogenetic sound change processes, where C*F*~0~ is often assumed to have transphonologized into lexical tone with concomitant loss of the voiced--voiceless contrast [@hyman1976; @kirby2017].
...
C*F*~0~ was first observed by @meyer1897 and first studied in English by @house1953. It has been documented in a very wide range of languages [see e.g. the overview in @ting2023].
...
Two plausible reasons for these discrepancies are 1) that languages use different articulatory mechanisms to implement post-aspiration, and 2) that the range of possible C*F*~0~ effects are limited when the phonological context places competing demands on the larynx, e.g. to produce a certain pitch level.
...
*RQ*1: What will C*F*~0~ look like in a language which contrasts two series of voiceless stops? (cp. @sec-cf0overview and @sec-review-danish-contrast)
...
* *H*1~c~: *F*~0~ is raised after both stop series, since both stop series are phonetically voiceless. The *F*~0~ levels following consonants would then be ranked either /p ~ b/ > /m/ or /p/ > /b/ > /m/, depending on whether the magnitude of devoicing gestures is expected to have an observable influence on C*F*~0~.
...
* *H*2~a~: C*F*~0~ is generally enhanced in speakers from Jutland relative to speakers from Zealand when there is a high pitch target, and high pitch targets enhance C*F*~0~ relative to low pitch targets in speakers from Jutland but not Zealand. This would suggest that focus in itself does not affect C*F*~0~, but a high pitch *does*, since high pitch targets are realized on the post-tonic syllable in Zealand Danish.
* *H*2~b~: C*F*~0~ is enhanced in global high pitch targets in speakers from both Jutland and Zealand. This would suggest that the global pitch target affects C*F*~0~, but focus in itself does not.
* *H*2~c~: Both varieties have similar C*F*~0~ effects, and C*F*~0~ is not affected by the global pitch target in either variety. This would suggest that there is no synergy.
...
The findings are in line with *H*1~c~ (devoicing generally raises *F*~0~) and are clear evidence *H*1~a~ (a phonological [+voice] specification in /b d g/ lowers *F*~0~).
...
*F*~0~ is increased after aspirated stops in languages like English, German, and Swedish, as well as Danish, but lowered after aspirated stops in languages like Nepali and Bengali. 
...
The upshot is that we do not find consistent evidence of co-intrinsic pitch being enhanced in high pitch environments, but rather that co-intrinsic pitch is inhibited or enhanced based on the source of high pitch.
